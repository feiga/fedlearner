...
# read from hdfs
#CUDA_VISIBLE_DEVICES="" doas python follower.py --local-addr=localhost:6008 --peer-addr=localhost:6009 --data-path=${HDFS_DATA_PATH} --use-hdfs True --start-time ${STRAT_DATE} --end-time ${END_DATE} > log_follower.txt 2>&1  &

python ../../fedlearner/trainer/parameter_server.py localhost:7002 > log_ps_follower.txt 2>&1 &
CUDA_VISIBLE_DEVICES="" doas python follower.py \
    --ps-addrs=localhost:7002 \
    --tf-addr=localhost:7012 \
    --local-addr=localhost:6008 \
    --peer-addr=localhost:6009 \
