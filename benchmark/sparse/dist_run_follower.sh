...
# read from hdfs
#CUDA_VISIBLE_DEVICES="" doas python follower.py --local-addr=localhost:5008 --peer-addr=localhost:5009 --data-path=${HDFS_DATA_PATH} --sparse-estimator True  --use-hdfs True --start-time ${STRAT_DATE} --end-time ${END_DATE} > log_follower.txt 2>&1  &

python ../../fedlearner/trainer/parameter_server.py localhost:8002 > log_ps_follower.txt 2>&1 &
CUDA_VISIBLE_DEVICES="" doas python follower.py \
    --ps-addrs=localhost:8002 \
    --tf-addr=localhost:8012 \
    --local-addr=localhost:5008 \
    --peer-addr=localhost:5009 \
    --data-path=${HDFS_DATA_PATH} \
    --sparse-estimator True  \
    --use-hdfs True \
    --start-time ${STRAT_DATE} \
    --end-time ${END_DATE} > log_follower.txt 2>&1  &
