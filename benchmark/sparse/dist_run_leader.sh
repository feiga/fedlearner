...
# read from hdfs
#CUDA_VISIBLE_DEVICES="" doas python leader.py --local-addr=localhost:5009 --peer-addr=localhost:5008 --data-path=${HDFS_DATA_PATH}  --sparse-estimator True --use-hdfs True --start-time ${STRAT_DATE} --end-time ${END_DATE} > log_leader.txt 2>&1 &
python ../../fedlearner/trainer/parameter_server.py localhost:8001 > log_ps_leader.txt 2>&1 &
CUDA_VISIBLE_DEVICES="" doas python leader.py \
    --ps-addrs=localhost:8001 \
    --tf-addr=localhost:8011 \
    --local-addr=localhost:5009 \
    --peer-addr=localhost:5008 \
